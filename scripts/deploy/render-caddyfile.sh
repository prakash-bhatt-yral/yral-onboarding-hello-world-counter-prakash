...
TEMPLATE_PATH="${APP_DIR}/caddy/Caddyfile.template"
...
if [[ ! -f "${TEMPLATE_PATH}" ]]; then
  fail "missing Caddy template at ${TEMPLATE_PATH}"
fi
...
awk -v tls_directive="${TLS_DIRECTIVE}" '{gsub(/__TLS_DIRECTIVE__/, tls_directive); print}' \
